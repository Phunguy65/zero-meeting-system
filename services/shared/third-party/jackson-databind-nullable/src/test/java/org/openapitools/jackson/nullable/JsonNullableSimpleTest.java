...
final class JsonNullableSimpleTest {
    ...
    void serializeAlways(JsonProcessor jsonProcessor) throws Exception {
        assertEquals(
                "{}",
                jsonProcessor.writeValueAsString(new Pet().name(JsonNullable.<String>undefined())));
        assertEquals("{\"name\":null}", jsonProcessor.writeValueAsString(new Pet().name(null)));
        assertEquals(
                "{\"name\":null}",
                jsonProcessor.writeValueAsString(new Pet().name(JsonNullable.<String>of(null))));
        assertEquals(
                "{\"name\":\"Rex\"}",
                jsonProcessor.writeValueAsString(new Pet().name(JsonNullable.of("Rex"))));
    }
    ...
    void serializeCollection(JsonProcessor jsonProcessor) throws Exception {
        assertEquals(
                "[\"foo\",null,null,null]",
                jsonProcessor.writeValueAsString(Arrays.asList(
                        JsonNullable.of("foo"),
    ...
    void deserializeStringNonBeanMembers(JsonProcessor jsonProcessor) throws Exception {
        assertEquals(
                JsonNullable.of(null),
                jsonProcessor.readValue("null", TypeReferences.STRING.getType(jsonProcessor)));
        assertEquals(
                JsonNullable.of("42"),
                jsonProcessor.readValue("\"42\"", TypeReferences.STRING.getType(jsonProcessor)));
        assertEquals(
                JsonNullable.of(""),
                jsonProcessor.readValue("\"\"", TypeReferences.STRING.getType(jsonProcessor)));
        assertEquals(
                JsonNullable.of("   "),
                jsonProcessor.readValue("\"   \"", TypeReferences.STRING.getType(jsonProcessor)));
    }
    ...
    void deserializeNonStringNonBeanMembers(JsonProcessor jsonProcessor) throws Exception {
        assertEquals(
                JsonNullable.of(null),
                jsonProcessor.readValue("\"null\"", TypeReferences.INTEGER.getType(jsonProcessor)));
        assertEquals(
                JsonNullable.of(42),
                jsonProcessor.readValue("\"42\"", TypeReferences.INTEGER.getType(jsonProcessor)));
        assertEquals(
                JsonNullable.undefined(),
                jsonProcessor.readValue("\"\"", TypeReferences.INTEGER.getType(jsonProcessor)));
        assertEquals(
                JsonNullable.undefined(),
                jsonProcessor.readValue("\"  \"", TypeReferences.INTEGER.getType(jsonProcessor)));
    }
    ...
    void deserializeCollection(JsonProcessor jsonProcessor) throws Exception {
        List<JsonNullable<String>> values = jsonProcessor.readValue(
                "[\"foo\", null]", TypeReferences.LIST_NULLABLE_STRING.getType(jsonProcessor));
        assertEquals(2, values.size());
        assertEquals(JsonNullable.of("foo"), values.get(0));
        assertEquals(JsonNullable.of(null), values.get(1));
    }

    private void testReadPetName(
            JsonProcessor jsonProcessor, JsonNullable<String> expected, String json)
            throws Exception {
    ...
    private void testReadPetAge(
            JsonProcessor jsonProcessor, JsonNullable<Integer> expected, String json)
            throws Exception {
        Pet pet = jsonProcessor.readValue(json, Pet.class);
