...
public class ListJoinRequestsUseCase {
    ...
    public Result<List<JoinRequestResponse>, MeetingError> execute(UUID meetingId, UUID requesterId) {
        ...
        if (!meeting.getHostId().equals(UserId.of(requesterId))) {
            return Result.failure(
                    new MeetingError.NotAuthorized(requesterId, meeting.getHostId().value()));
        }

        List<JoinRequestResponse> responses = joinRequestRepository.findPendingByMeetingId(meetingId)
                .stream()
